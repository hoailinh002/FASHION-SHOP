...
function Header () {
    const [menu] = useState(["SẢN PHẨM", "BỘ SƯU TẬP", "SẢN PHẨM GIÁ TỐT", "SALE"]);
    const urls = ["/Products", "/collections", "/good-deals", "/sale"]; 
    return (
        <div className="">

            <div className="h-28 header flex items-center">

                <div className="w-1/3 mx-auto text-5xl">NHL</div>
                <div className="flex-1">
                    {menu.map((menu, index)=>(
                        <Link className='mx-8' to={urls[index]}>{menu}</Link>
                    ))}
                </div>
